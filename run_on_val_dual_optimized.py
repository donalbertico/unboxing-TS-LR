...
def run_cv_pooled_dual_optimized(x_filt_data, y, groups):
    ...
    # Store results for all combinations: (ch, band) -> list of (score, size)
    ...
    for train_idx, test_idx in cv.split(x_covs[0], y, groups=groups):
        ...
        # 1. Ranking Phase (Fit once per fold)
...
def process_subject(sub):
    ...
    try:
        ...
        x_norm = filter_data_to_numpy(epochs_data, 0.1, 1.8, MAX_FREQ_DEFAULT)
        acc_grid = run_cv_pooled_dual_optimized(x_norm, labels, groups)
        return sub, acc_grid
...
def run_all():
    ...
    with open('bcicomp_dual_optimized_grid.pkl', 'wb') as f:
        pickle.dump(results, f)
    print('Saved: bcicomp_dual_optimized_grid.pkl')
